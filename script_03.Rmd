...
```{r}
sep_new <- 
  read_excel("class_enrollment_summary_by_term_9-21-2020.xlsx",
             skip = 3) %>%
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
...
- what data will we turn to.
- what we are seeing is a drop in students, add/drop process, or change in class 
- relevant estimand 
- what data do we want to acquire
- 

After we have a model, Temperance suggests we should: 

- model is simplied, won't tell us detailed information 
- 
